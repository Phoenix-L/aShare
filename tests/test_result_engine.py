...
def test_build_summary_omits_irrelevant_shock_columns(monkeypatch, tmp_path: Path) -> None:
    ...
    output_root.mkdir(parents=True, exist_ok=True)
    _write_run(output_root, "run_001", metrics={"sharpe": 1.1, "total_return": 0.08, "max_drawdown": 0.05, "num_trades": 3}, parameters={"use_trend_filter": False, "excursion_lookback_bars": 3, "excursion_threshold": 0.01, "trend_ma_period": 2, "take_profit_pct": 0.02, "recovery_frac": 0.5, "max_hold_bars": 10, "stop_loss_pct": 0.1}, meta={"strategy": "shock_reversion_intraday"})
