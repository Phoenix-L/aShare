...
def test_collect_and_build_summary_outputs_csv(monkeypatch, tmp_path: Path) -> None:
    ...
    _write_run(
        ...
        parameters={"z_entry": -1.5, "z_exit": 1.0, "use_trend_filter": True, "use_art_filter": False},
    )
    _write_run(
        output_root,
        "run_002",
        metrics={"sharpe": 1.8, "total_return": 0.25, "max_drawdown": 0.10, "num_trades": 10},
        parameters={"z_entry": -2.0, "z_exit": 1.2, "use_trend_filter": False, "use_art_filter": True},
    )
    ...
    assert "z_entry,z_exit,use_trend_filter,use_art_filter,total_return,sharpe,max_drawdown,num_trades" in summary_text
