...
def test_shock_reversion_score_filter_is_counted_in_diagnostics_summary() -> None:
    ...
    summary = metrics["diagnostics_summary"]
    blocked = [row for row in strat.diagnostics if row["entry_signal"] and row["blocked_by_shock_score_low"]]

    assert blocked
    assert summary["blocked_by_shock_score_low"] == len(blocked)
    assert summary["blocked_by_shock_score_high"] == 0
    assert "blocked_by_trend" not in summary
    assert "blocked_by_atr" not in summary
    assert "blocked_by_art" not in summary


def test_shock_reversion_score_filter_high_blocks_are_counted_in_diagnostics_summary() -> None:
    closes = [100.0] * 180 + [95.0, 96.0, 97.0, 98.0]
    _, strat, metrics = run_backtest(
        strategy_cls=ShockReversionIntradayStrategy,
        data_df=_synthetic_df(closes),
        config=_config(),
    ...
    summary = metrics["diagnostics_summary"]
    blocked = [row for row in strat.diagnostics if row["entry_signal"] and row["blocked_by_shock_score_high"]]

    assert blocked
    assert summary["blocked_by_shock_score_high"] == len(blocked)
    assert summary["blocked_by_shock_score_low"] == 0
