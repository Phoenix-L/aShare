...
def test_zscore_signal_executes_without_excursion_coupling() -> None:
    closes = [100.0] * 180 + [97.0, 97.0]
    strat = _run(closes, {"trade_unit": 500, "z_entry": -1.0, "z_exit": 5.0, "use_trend_filter": False, "use_atr_filter": False}, spread=1.0)
    assert strat.buy_events >= 1
    assert any(row["executed"] for row in strat.diagnostics)
    assert all("excursion" not in row for row in strat.diagnostics)


def test_mean_reversion_atr_and_legacy_art_filter_params_behave_the_same() -> None:
    closes = [100.0] * 180 + [99.0, 99.0]
    strat_atr = _run(closes, {"trade_unit": 500, "z_entry": -0.5, "z_exit": 5.0, "use_trend_filter": False, "use_atr_filter": True}, spread=0.01)
    strat_art = _run(closes, {"trade_unit": 500, "z_entry": -0.5, "z_exit": 5.0, "use_trend_filter": False, "use_art_filter": True}, spread=0.01)
    assert strat_atr.buy_events == 0
    assert strat_art.buy_events == 0


def test_mean_reversion_rejects_excursion_related_params() -> None:
    closes = [100.0] * 180 + [97.0, 97.0]
    with pytest.raises(ValueError, match="does not accept excursion"):
        _run(closes, {"trade_unit": 500, "z_entry": -1.0, "z_exit": 5.0, "use_trend_filter": False, "use_atr_filter": False, "use_multi_day_excursion": True})
